							...
							void (*copy)(void * dest, void * array, size_t elm_size))
{
	...
	while (le < left_end && ri < arr_size)
	{
		...
		{
			memcpy(&tmp[ind_merge*elm_size], &array[le*elm_size], elm_size);
			++le, ++ind_merge;
		}
		else
		{
			memcpy(&tmp[ind_merge*elm_size], &array[ri*elm_size], elm_size);
			++ri, ++ind_merge;
		}	
	}
	...
	memcpy(arr, tmp, elm_size * ind_merge);
...
{		
	if (arr_size > 1)
	{
		...
		merge_sort_mcpy(arr, half_size, elm_size, compar, copy);
		merge_sort_mcpy(second_half, arr_size - half_size, elm_size, compar, copy);
